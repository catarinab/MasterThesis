...
int arnoldiIteration(const csr_matrix& A, dense_vector& initVec, int k_total, int m, int me, dense_matrix * V,
    ...
    dense_matrix Z(counts[me], k_total + l + 1);
    dense_matrix G(counts[me], k_total + l);
    double * vCol, * zCol, *xZcol;
    auto vValVec = new double[counts[me]]();
    auto dotProds = new double[k_total + l];
    MPI_Request r1, r2;
        ...
        if (a >= 0) {
            ...
            for (int j = a - l + 2; j <= a; j++) {
                ...
                for (int k = 0; k <= j - 1; k++) {
                    ...
                    hVal1 += G.getValue(k, j) * G.getValue(k, a + 1);
                }
                //fprintf(stderr, "Getting G(%d, %d) ( = %f) \n", j, j, G.getValue(j, j));
                G.setValue(j, a + 1, (G.getValue(j, a + 1) - hVal1) / G.getValue(j, j));
            ...
            if (a < l) {
                for (int j = 0; j <= a; j++) {
                    ...
                    for (int k = 0; k <= a - 1; k++) {
                        ...
                        temp += H->getValue(j, k) * G.getValue(k, a);
                    }
                    //fprintf(stderr, "getting G(%d, %d), G(%d, %d) and G(%d, %d)\n", j, a+1, j, a, a,a);
                    double val = (G.getValue(j, a + 1) + G.getValue(j, a) - temp) / G.getValue(a, a);
                    H->setValue(j, a, val);
                ...
                for (int j = 0; j <= a; j++) {
                    for (int k = 0; k <= a + 1 - l; k++) {
                        ...
                        hVal1 += G.getValue(j, k + l) * H->getValue(k, a - l);
                    }
                    for (int k = j - 1; k <= a - 1; k++) {
                        //fprintf(stderr, "getting H(%d, %d) and G(%d, %d)\n", j, k, k, a);
                        hVal2 += H->getValue(j, k) * G.getValue(k, a);
                    }
                    //fprintf(stderr, "getting G(%d, %d)\n", a, a);
                    H->setValue(j, a, (hVal1 - hVal2) / G.getValue(a, a));
                    //fprintf(stderr, "Setting H(%d, %d) to %f\n", j, a, H->getValue(j, a));
                }
            ...
            for (int j = 0; j <= a - 1; j++) {
                ...
                for (int idx = 0; idx < counts[me]; idx++) {
                    vValVec[idx] += vCol[idx] * G.getValue(j, a);
                }
            }
            ...
            for (int j = 0; j <= a - 1; j++) {
                ...
                for (int idx = 0; idx < counts[me]; idx++) {
                    vValVec[idx] += zCol[idx] * H->getValue(j, a - 1);
                }
            }
            if(a > 0) {
                Z.getCol(i + 1, &zCol);
