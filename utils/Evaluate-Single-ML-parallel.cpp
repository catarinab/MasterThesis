...
complex<double> LTI(complex<double> lambda, double alpha, double beta, int k, int numThreads){
    ...
    #pragma omp parallel for if(numThreads > 0) num_threads(numThreads) reduction(+:result)
    for (int j = 0; j <= k - p; j++) {
        if (abs(c[k - p][j]) > tau) {
...
complex<double> evaluateSingle(complex<double> z, double alpha, double beta, int k, int numThreads) {
    ...
    if(!accept){
        result = LTI(tVal, alpha, beta, k, numThreads);
    }
